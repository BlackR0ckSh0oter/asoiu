...
void __fastcall TForm8::SetData(TADOTable *Table)
{
	...
	EditDate->Text = Table->FieldByName("Дата_начала_брони")->AsString;
	EditTime->Text = Table->FieldByName("Дата_окончания_брони")->AsString;

    // Настройка DBLookupComboBox1 для выбора сотрудников
    DBLookupComboBox1->ListSource = DataSource2; // Список сотрудников
	DBLookupComboBox1->ListField = "Тип_номера"; // Поле, отображающее ФИО
	DBLookupComboBox1->KeyField = "ID_номера"; // Поле, содержащее ID сотрудника
	DBLookupComboBox1->KeyValue = Table->FieldByName("ID_номера")->AsInteger; // Установите текущее значение
 ...
 try
	{
		...
		ADOTable3->Edit();
		ADOTable3->FieldByName("Дата_начала_брони")->AsString = EditDate->Text;
		ADOTable3->FieldByName("Дата_окончания_брони")->AsString = EditTime->Text;

		int selectedID = DBLookupComboBox1->KeyValue;
		ADOTable3->FieldByName("ID_номера")->AsInteger = selectedID;
